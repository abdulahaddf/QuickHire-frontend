   ...
   {/* Background Pattern */}
      ...
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-20" >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {displayJobs.map((job) => (
            <Link key={job.id} href={`/jobs/${job.id}`} className="flex items-center p-6 bg-white border border-gray-100 hover:border-blue-500 hover:shadow-md transition-all min-w-0">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-xl font-bold text-gray-900 mb-1 hover:text-blue-600 transition-colors truncate">
                  {job.title}
                </h3>
                <p className="text-gray-500 text-sm font-medium mb-3 truncate">{job.company} • {job.location}</p>
